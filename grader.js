...
var loadUrl = function(url,checksfile) {
    var loadit = function(result, response) {
	    if(result instanceof Error) {            
		console.error('Error: ' + util.format(response.message));
            } else {
		console.error("Read %s", url);
...
var checkHtmlFile =  function(htmlfile, checksfile, isUrl) {
    if(isUrl) {
	rest.get(htmlfile).on('complete', loadUrl(htmlfile,checksfile));
    } else {
	doChecks(checksfile,fs.readFileSync(htmlfile));
    }
};
var doChecks = function(checksfile,text) {
    $ = cheerio.load(text);
